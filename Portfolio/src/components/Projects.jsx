        ...
        <div className='font-custom bg-lightBlack text-lightGrey py-20 xl:px-32 lg:px-20 md:px-12 px-6'>
          <div className='md:pb-10 pt-20'>
            <nav className='flex justify-between '>
              <h1 className='text-[35px] md:text-[72px]'>Projects</h1>
              <h3 className="text-[15px] text-[500] leading-[28px] tracking-[2.3px] py-3 md:py-12 lg:py-14 underline hover:text-cyan underline-offset-8  decoration-cyan "><Link to="/contactforms">CONTACT ME</Link></h3>
            </nav>
          </div>
          <div className='cards grid grid-cols-1 md:grid-cols-2 gap-10'>
            {project.map((item,index) => (
              ...
              onMouseLeave={ handleMouseLeave}>
                <Card  key={item} item={item} isHovered={isHovered === index}/>
              </div>
            ))}
          </div>
        </div>
